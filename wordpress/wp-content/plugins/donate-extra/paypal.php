...
if(class_exists('DonateExtra'))$donateextra = new DonateExtra();

#1 = Live on PayPal Network
#2 = Testing PayPal Sandbox
$verifymode 	= $dextra['testing_mode']; # be sure to change value for testing/live!
# Send notifications to here
$send_mail_to 	= $dextra['ty_email'];
# subject of messages
$sysname 		= "Donate Extra - Paypal IPN Transaction";
# Your primary PayPal e-mail address
//$paypal_email 	= $dextra['paypal_email'];
# Your sendmail path
//$mailpath 		= "/usr/sbin/sendmail -t";
#the name you wish to see the messages from
//$from_name 		= $dextra['ty_name'];
#the emails will be coming from
//$from_email 	= $dextra['ty_email'];


# Convert Super globals For backward compatibility
if(phpversion() <= "4.0.6") {$_POST=($HTTP_POST_VARS);}

if (!isset($_POST['txn_type']) && isset($_REQUEST['txn_type'])) {
	$_POST = $_REQUEST;
}
...
}elseif ($verifymode == 2){	//2 = Testing with the PayPal Sandbox
...
if (!$port && !$error){
	...
	if(!strcmp($reply, "VERIFIED")){
		...
		if($tmp_count == 0){
			...
			global $currency;
			$subject 	= stripslashes($dextra['ty_subject']);
			$prefix 	= $currency[$mc_currency]['symbol'];
			$amount 	= $prefix.$mc_gross.' '.$mc_currency;
			$payer_msg 	= nl2br($donateextra->TagReplace(stripslashes($dextra['ty_emailmsg']), $option_name1, $amount));
			//$payer_msg = utf8_encode($payer_msg);
			//echo '<br />'.$payer_msg;
			$headers  = 'MIME-Version: 1.0'."\r\n";
			//$headers .= 'Content-type: text/html; charset=iso-8859-1'."\r\n";
			$headers .= 'Content-type: text/html; charset=utf-8'."\r\n";
			$headers .= 'From: '.$dextra['ty_name'].' <'.$dextra['ty_email'].'>'."\r\n";
