...
global $wpdb;
	...
	if ( $wppa['is_topten'] && ! wppa_switch('wppa_bc_on_topten') ) return;		// Any special selection has its own switch
	...
	$wppa['out'] .= wppa_nltab('+').'<div id="wppa-bc-'.$wppa['master_occur'].'" class="wppa-nav wppa-box wppa-nav-text" style="'.__wcs('wppa-nav').__wcs('wppa-box').__wcs('wppa-nav-text').'">';
		...
		if ( wppa_switch('wppa_show_page') ) {
			...
			$href	= ( $alb || $virtual || $wppa['src'] || $is_albenum ) ? wppa_get_permalink($pid, true) : '';
		...
		elseif ( $wppa['is_topten'] ) {							// TopTen
			...
			if ( $wppa['is_slide'] ) {
				...
				$href 	= wppa_get_permalink().'wppa-cover=0&amp;wppa-occur='.$wppa['occur'].'&amp;wppa-topten='.$wppa['topten_count'].'&amp;wppa-album='.$wppa['start_album'];
				$title	= __a('View the thumbnails');
				wppa_bcitem($value, $href, $title, 'b8');
			}
			$value 	= __a('Top rated photos');
		...
		elseif ( $wppa['is_lasten'] ) {							// Lasten
			...
			if ( $wppa['is_slide'] ) {
				...
				$href 	= wppa_get_permalink().'wppa-cover=0&amp;wppa-occur='.$wppa['occur'].'&amp;wppa-lasten='.$wppa['lasten_count'].'&amp;wppa-album='.$wppa['start_album'];
				$title	= __a('View the thumbnails');
				wppa_bcitem($value, $href, $title, 'b8');
			}
			$value 	= __a('Recently uploaded photos');
		...
		elseif ( $wppa['is_featen'] ) {							// Featen
			...
			if ( $wppa['is_slide'] ) {
				...
				$href 	= wppa_get_permalink().'wppa-cover=0&amp;wppa-occur='.$wppa['occur'].'&amp;wppa-featen='.$wppa['featen_count'].'&amp;wppa-album='.$wppa['start_album'];
				$title	= __a('View the thumbnails');
				wppa_bcitem($value, $href, $title, 'b8');
			}
			$value 	= __a('Featured photos');
...
// Display a breadcrumb item with optionally a seperator if it is a link. If its a link, its not the last item
...
static $sep;
	...
	if ( $href ) {
		$wppa['out'] .= '<a href="'.$href.'" class="wppa-nav-text '.$class.'" style="'.__wcs('wppa-nav-text').'" title="'.$title.'" >'.$value.'</a>';
	}
	else {					// No link, its the last item
		$wppa['out'] .= '<span id="bc-pname-'.$wppa['master_occur'].'" class="wppa-nav-text '.$class.'" style="'.__wcs('wppa-nav-text').'" title="'.$title.'" >'.$value.'</span>';
		return;
	}
