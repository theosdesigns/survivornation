	...
	<div id="primary" class="content-area <?php echo $firmasite_settings["layout_primary_class"]; ?>">
		<div class="padder">
			...
			<form action="" method="post" id="forums-search-form" class="dir-form">
				...
				<h3 class="page-header"><?php _e( 'Forums Directory', 'firmasite' ); ?><?php if ( is_user_logged_in() ) : ?> &nbsp;<a class="button btn  show-hide-new" href="#new-topic" id="new-topic-button"><?php _e( 'New Topic', 'firmasite' ); ?></a><?php endif; ?></h3>
			...
			<form action="" method="post" id="forums-directory-form" class="dir-form">
				...
				<div class="item-list-tabs" role="navigation">
					<ul class="nav nav-pills">
						<li class="selected" id="forums-all"><a href="<?php echo trailingslashit( bp_get_root_domain() . '/' . bp_get_forums_root_slug() ); ?>"><?php printf( __( 'All Topics <span>%s</span>', 'firmasite' ), bp_get_forum_topic_count() ); ?></a></li>

						<?php if ( is_user_logged_in() && bp_get_forum_topic_count_for_user( bp_loggedin_user_id() ) ) : ?>

							<li id="forums-personal"><a href="<?php echo trailingslashit( bp_loggedin_user_domain() . bp_get_forums_slug() . '/topics' ); ?>"><?php printf( __( 'My Topics <span>%s</span>', 'firmasite' ), bp_get_forum_topic_count_for_user( bp_loggedin_user_id() ) ); ?></a></li>
				...
				<div class="item-list-tabs" id="subnav" role="navigation">
					<ul class="nav nav-pills">
						...
						<li id="forums-order-select" class="last pull-right filter">
				...
				<?php if ( is_user_logged_in() ) : ?>
					...
					<?php if ( bp_is_active( 'groups' ) && bp_has_groups( 'user_id=' . bp_loggedin_user_id() . '&type=alphabetical&max=100&per_page=100' ) ) : ?>
						...
						<form action="" method="post" id="forum-topic-form" class="standard-form">
							...
							<a name="post-new"></a>
							<h5><?php _e( 'Create New Topic:', 'firmasite' ); ?></h5>
							...
							<label><?php _e( 'Post In Group Forum:', 'firmasite' ); ?></label>
							<select id="topic_group_id" name="topic_group_id">

								<option value=""><?php /* translators: no option picked in select box */ _e( '----', 'firmasite' ); ?></option>
							...
							<div class="submit">
								<input type="submit" class="btn  btn-primary" name="submit_topic" id="submit" value="<?php _e( 'Post Topic', 'firmasite' ); ?>" />
								<input type="button" class="btn"  name="submit_topic_cancel" id="submit_topic_cancel" value="<?php _e( 'Cancel', 'firmasite' ); ?>" />
							</div>
					...
					<?php elseif ( bp_is_active( 'groups' ) ) : ?>
						...
						<div id="message" class="info">
							...
							<p><?php printf( __( "You are not a member of any groups so you don't have any group forums you can post in. To start posting, first find a group that matches the topic subject you'd like to start. If this group does not exist, why not <a href='%s'>create a new group</a>? Once you have joined or created the group you can post your topic in that group's forum.", 'firmasite' ), site_url( bp_get_groups_root_slug() . '/create/' ) ); ?></p>
