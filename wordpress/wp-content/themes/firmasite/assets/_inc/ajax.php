...
function bp_dtheme_post_update() {
	...
	if ( empty( $_POST['content'] ) )
		exit( '-1<div id="message" class="error"><p>' . __( 'Please enter some content to post.', 'firmasite' ) . '</p></div>' );
	...
	if ( empty( $activity_id ) )
		exit( '-1<div id="message" class="error"><p>' . __( 'There was a problem posting your update, please try again.', 'firmasite' ) . '</p></div>' );
...
function bp_dtheme_new_activity_comment() {
	...
	if ( empty( $_POST['content'] ) )
		exit( '-1<div id="message" class="error"><p>' . __( 'Please do not leave the comment area blank.', 'firmasite' ) . '</p></div>' );

	if ( empty( $_POST['form_id'] ) || empty( $_POST['comment_id'] ) || ! is_numeric( $_POST['form_id'] ) || ! is_numeric( $_POST['comment_id'] ) )
		exit( '-1<div id="message" class="error"><p>' . __( 'There was an error posting that reply, please try again.', 'firmasite' ) . '</p></div>' );
...
function bp_dtheme_delete_activity() {
	...
	if ( ! bp_activity_delete( array( 'id' => $activity->id, 'user_id' => $activity->user_id ) ) )
		exit( '-1<div id="message" class="error"><p>' . __( 'There was a problem when deleting. Please try again.', 'firmasite' ) . '</p></div>' );
...
function bp_dtheme_ajax_invite_user() {
	...
	if ( 'invite' == $_POST['friend_action'] ) {
		...
		echo '<div class="action">
				<a class="button remove" href="' . wp_nonce_url( bp_loggedin_user_domain() . bp_get_groups_slug() . '/' . $_POST['group_id'] . '/invites/remove/' . $user->id, 'groups_invite_uninvite_user' ) . '" id="uid-' . esc_attr( $user->id ) . '">' . __( 'Remove Invite', 'firmasite' ) . '</a>
			  </div>';
		echo '</li>';
		exit;
...
function bp_dtheme_ajax_addremove_friend() {
	...
	if ( 'is_friend' == BP_Friends_Friendship::check_is_friend( bp_loggedin_user_id(), $_POST['fid'] ) ) {
		...
		if ( ! friends_remove_friend( bp_loggedin_user_id(), $_POST['fid'] ) )
			...
			echo '<a id="friend-' . $_POST['fid'] . '" class="add" rel="add" title="' . __( 'Add Friend', 'firmasite' ) . '" href="' . wp_nonce_url( bp_loggedin_user_domain() . bp_get_friends_slug() . '/add-friend/' . $_POST['fid'], 'friends_add_friend' ) . '">' . __( 'Add Friend', 'firmasite' ) . '</a>';
		...
		if ( ! friends_add_friend( bp_loggedin_user_id(), $_POST['fid'] ) )
			...
			echo '<a id="friend-' . $_POST['fid'] . '" class="remove" rel="remove" title="' . __( 'Cancel Friendship Request', 'firmasite' ) . '" href="' . wp_nonce_url( bp_loggedin_user_domain() . bp_get_friends_slug() . '/requests/cancel/' . (int) $_POST['fid'] . '/', 'friends_withdraw_friendship' ) . '" class="requested">' . __( 'Cancel Friendship Request', 'firmasite' ) . '</a>';
...
function bp_dtheme_ajax_accept_friendship() {
	...
	if ( ! friends_accept_friendship( $_POST['id'] ) )
		echo "-1<div id='message' class='error'><p>" . __( 'There was a problem accepting that request. Please try again.', 'firmasite' ) . '</p></div>';
...
function bp_dtheme_ajax_reject_friendship() {
	...
	if ( ! friends_reject_friendship( $_POST['id'] ) )
		echo "-1<div id='message' class='error'><p>" . __( 'There was a problem rejecting that request. Please try again.', 'firmasite' ) . '</p></div>';
	...
	if ( ! groups_is_user_member( bp_loggedin_user_id(), $group->id ) ) {
		if ( 'public' == $group->status ) {
			...
			if ( ! groups_join_group( $group->id ) )
				...
				echo '<a id="group-' . esc_attr( $group->id ) . '" class="leave-group" rel="leave" title="' . __( 'Leave Group', 'firmasite' ) . '" href="' . wp_nonce_url( bp_get_group_permalink( $group ) . 'leave-group', 'groups_leave_group' ) . '">' . __( 'Leave Group', 'firmasite' ) . '</a>';
			...
			if ( ! groups_send_membership_request( bp_loggedin_user_id(), $group->id ) )
				...
				echo '<a id="group-' . esc_attr( $group->id ) . '" class="membership-requested" rel="membership-requested" title="' . __( 'Membership Requested', 'firmasite' ) . '" href="' . bp_get_group_permalink( $group ) . '">' . __( 'Membership Requested', 'firmasite' ) . '</a>';
		}
		...
		elseif ( 'public' == $group->status )
			echo '<a id="group-' . esc_attr( $group->id ) . '" class="join-group" rel="join" title="' . __( 'Join Group', 'firmasite' ) . '" href="' . wp_nonce_url( bp_get_group_permalink( $group ) . 'join', 'groups_join_group' ) . '">' . __( 'Join Group', 'firmasite' ) . '</a>';
		elseif ( 'private' == $group->status )
			echo '<a id="group-' . esc_attr( $group->id ) . '" class="request-membership" rel="join" title="' . __( 'Request Membership', 'firmasite' ) . '" href="' . wp_nonce_url( bp_get_group_permalink( $group ) . 'request-membership', 'groups_send_membership_request' ) . '">' . __( 'Request Membership', 'firmasite' ) . '</a>';
	}
...
function bp_dtheme_ajax_close_notice() {
	...
	if ( ! isset( $_POST['notice_id'] ) ) {
		echo "-1<div id='message' class='error'><p>" . __( 'There was a problem closing the notice.', 'firmasite' ) . '</p></div>';
...
function bp_dtheme_ajax_messages_send_reply() {
	...
	<?php
		...
		echo "-1<div id='message' class='error'><p>" . __( 'There was a problem sending that reply. Please try again.', 'firmasite' ) . '</p></div>';
	}
...
function bp_dtheme_ajax_message_markunread() {
	...
	if ( ! isset($_POST['thread_ids']) ) {
		echo "-1<div id='message' class='error'><p>" . __( 'There was a problem marking messages as unread.', 'firmasite' ) . '</p></div>';
...
function bp_dtheme_ajax_message_markread() {
	...
	if ( ! isset($_POST['thread_ids']) ) {
		echo "-1<div id='message' class='error'><p>" . __('There was a problem marking messages as read.', 'firmasite' ) . '</p></div>';
...
function bp_dtheme_ajax_messages_delete() {
	...
	if ( ! isset($_POST['thread_ids']) ) {
		echo "-1<div id='message' class='error'><p>" . __( 'There was a problem deleting messages.', 'firmasite' ) . '</p></div>';
