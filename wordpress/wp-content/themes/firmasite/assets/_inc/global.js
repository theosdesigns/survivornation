...
function bp_init_activity() {
	...
	if ( null != jq.cookie('bp-activity-scope') && jq('.activity-type-tabs').length ) {
		...
		jq('li#activity-' + jq.cookie('bp-activity-scope') + ', .item-list-tabs li.current').addClass('selected active');
	}
}
...
function bp_init_objects(objects) {
	jq(objects).each( function(i) {
		...
		if ( null != jq.cookie('bp-' + objects[i] + '-scope') && jq('div.' + objects[i]).length ) {
			...
			jq('.item-list-tabs li#' + objects[i] + '-' + jq.cookie('bp-' + objects[i] + '-scope') + ', div.item-list-tabs#object-nav li.current').addClass('selected active');
		}
	});
}
...
function bp_filter_request( object, filter, scope, target, search_terms, page, extras ) {
	...
	jq('.item-list-tabs li#' + object + '-' + scope + ', .item-list-tabs#object-nav li.current').addClass('selected active');
	jq('.item-list-tabs li.selected').addClass('loading');
...
function bp_activity_request(scope, filter) {
	...
	/* Set the correct selected nav and filter */
	jq('li#activity-' + scope + ', .item-list-tabs li.current').addClass('selected active');
