...
<?php if ( bbp_current_user_can_access_create_forum_form() ) : ?>
	...
	<div id="new-forum-<?php bbp_forum_id(); ?>" class="bbp-forum-form well form-inline">
			...
			<fieldset class="bbp-form">
				<legend>
					...
					<?php
						if ( bbp_is_forum_edit() )
							printf( __( 'Now Editing &ldquo;%s&rdquo;', 'firmasite' ), bbp_get_forum_title() );
						else
							bbp_is_single_forum() ? printf( __( 'Create New Forum in &ldquo;%s&rdquo;', 'firmasite' ), bbp_get_forum_title() ) : _e( 'Create New Forum', 'firmasite' );
					?>
				...
				<?php if ( !bbp_is_forum_edit() && bbp_is_forum_closed() ) : ?>
					...
					<div class="bbp-template-notice alert alert-block alert-info">
						<p><?php _e( 'This forum is closed to new content, however your account still allows you to do so.', 'firmasite' ); ?></p>
					</div>
				...
				<div>
					...
					<p>
						<label for="bbp_forum_title"><?php printf( __( 'Forum Name (Maximum Length: %d):', 'firmasite' ), bbp_get_title_max_length() ); ?></label><br />
					...
					<?php if ( !function_exists( 'wp_editor' ) ) : ?>
						...
						<p>
							<label for="bbp_forum_content"><?php _e( 'Forum Description:', 'firmasite' ); ?></label><br />
					...
					<p>
						<label for="bbp_forum_type"><?php _e( 'Forum Type:', 'firmasite' ); ?></label><br />
					...
					<p>
						<label for="bbp_forum_status"><?php _e( 'Status:', 'firmasite' ); ?></label><br />
					...
					<p>
						<label for="bbp_forum_visibility"><?php _e( 'Visibility:', 'firmasite' ); ?></label><br />
					...
					<p>
						<label for="bbp_forum_parent_id"><?php _e( 'Parent Forum:', 'firmasite' ); ?></label><br />

						<?php
							bbp_dropdown( array(
								'select_id' => 'bbp_forum_parent_id',
								'show_none' => __( '(No Parent)', 'firmasite' ),
					...
					<div class="bbp-submit-wrapper">
						...
						<button type="submit" class="btn btn-primary" tabindex="<?php bbp_tab_index(); ?>" id="bbp_forum_submit" name="bbp_forum_submit" class="button submit"><?php _e( 'Submit', 'firmasite' ); ?></button>
...
<?php elseif ( bbp_is_forum_closed() ) : ?>
	...
	<div id="no-forum-<?php bbp_forum_id(); ?>" class="bbp-no-forum">
		<div class="bbp-template-notice alert alert-block alert-info">
			<p><?php printf( __( 'The forum &#8216;%s&#8217; is closed to new content.', 'firmasite' ), bbp_get_forum_title() ); ?></p>
		</div>
	</div>
...
<?php else : ?>
	...
	<div id="no-forum-<?php bbp_forum_id(); ?>" class="bbp-no-forum">
		<div class="bbp-template-notice alert alert-block alert-info">
			<p><?php is_user_logged_in() ? _e( 'You cannot create new forums.', 'firmasite' ) : _e( 'You must be logged in to create new forums.', 'firmasite' ); ?></p>
		</div>
	</div>
