...
<form action="<?php bp_forum_topic_action(); ?>" method="post" id="forum-topic-form" class="standard-form">
	<div class="item-list-tabs no-ajax" id="subnav" role="navigation">
		<ul class="nav nav-pills">
			<?php if ( is_user_logged_in() ) : ?>
				...
				<li>
					<a href="<?php bp_forum_topic_new_reply_link(); ?>" class="new-reply-link"><?php _e( 'New Reply', 'firmasite' ); ?></a>
				</li>
	...
	<div id="topic-meta">
		<h3 class="page-header"><?php bp_the_topic_title(); ?> (<?php bp_the_topic_total_post_count(); ?>)</h3>
	
		<?php if ( bp_forum_topic_has_tags() ) : ?>
	
			<div class="topic-tags">
	
				<?php _e( 'Topic tags:', 'firmasite' ); ?> <?php bp_forum_topic_tag_list(); ?>
		...
		<?php if ( bp_group_is_admin() || bp_group_is_mod() || bp_get_the_topic_is_mine() ) : ?>
			...
			<div class="last pull-right admin-links">
	...
	<?php if ( bp_has_forum_topic_posts() ) : ?>
		...
		<div class="pagination no-ajax">
			...
			<div class="pagination-links lead" id="topic-pag-top">
			...
			<?php while ( bp_forum_topic_posts() ) : bp_the_forum_topic_post(); ?>
				...
				<li id="post-<?php bp_the_topic_post_id(); ?>" class="<?php bp_the_topic_post_css_class(); ?>">
					<div class="poster-meta">
						...
						<?php echo sprintf( __( '%1$s said %2$s:', 'firmasite' ), bp_get_the_topic_post_poster_name(), bp_get_the_topic_post_time_since() ); ?>
					</div>
					...
					<div class="admin-links">
						...
						<a href="#post-<?php bp_the_topic_post_id(); ?>" title="<?php _e( 'Permanent link to this post', 'firmasite' ); ?>">#</a>
					</div>
				</li>
		...
		<div class="pagination no-ajax">
			...
			<div class="pagination-links lead" id="topic-pag-bottom">
				<?php bp_the_topic_pagination(); ?>
			</div>
	...
	<?php else: ?>
		...
		<div id="message" class="info">
			<p><?php _e( 'There are no posts for this topic.', 'firmasite' ); ?></p>
		</div>
	...
	<?php if ( ( is_user_logged_in() && 'public' == bp_get_group_status() ) || bp_group_is_member() ) : ?>
		...
		<?php if ( bp_get_the_topic_is_last_page() ) : ?>
			...
			<?php if ( bp_get_the_topic_is_topic_open() && !bp_group_is_user_banned() ) : ?>
				...
				<div id="post-topic-reply">
					...
					<?php if ( bp_groups_auto_join() && !bp_group_is_member() ) : ?>
						<p><?php _e( 'You will auto join this group when you reply to this topic.', 'firmasite' ); ?></p>
					<?php endif; ?>

					<?php do_action( 'groups_forum_new_reply_before' ); ?>

					<h4 class="page-header"><?php _e( 'Add a reply:', 'firmasite' ); ?></h4>

					<textarea name="reply_text" id="reply_text"></textarea>

					<div class="submit">
						<input type="submit" class="btn  btn-primary" name="submit_reply" id="submit" value="<?php _e( 'Post Reply', 'firmasite' ); ?>" />
					</div>
			...
			<?php elseif ( !bp_group_is_user_banned() ) : ?>
				...
				<div id="message" class="info">
					<p><?php _e( 'This topic is closed, replies are no longer accepted.', 'firmasite' ); ?></p>
				</div>
