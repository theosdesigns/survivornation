...
get_header( 'buddypress' ); ?>
	...
	<div id="primary" class="content-area <?php echo $firmasite_settings["layout_primary_class"]; ?>">
		...
		<form action="<?php bp_group_creation_form_action(); ?>" method="post" id="create-group-form" class="standard-form" enctype="multipart/form-data">
			<h3 class="page-header"><?php _e( 'Create a Group', 'firmasite' ); ?> &nbsp;<a class="button btn " href="<?php echo trailingslashit( bp_get_root_domain() . '/' . bp_get_groups_root_slug() ); ?>"><?php _e( 'Groups Directory', 'firmasite' ); ?></a></h3>
			...
			<div class="item-body" id="group-create-body">
				...
				<?php if ( bp_is_group_creation_step( 'group-avatar' ) ) : ?>
					...
					<?php if ( 'upload-image' == bp_get_avatar_admin_step() ) : ?>
						...
						<div class="main-column">
							...
							<p><?php _e( 'To skip the avatar upload process, hit the "Next Step" button.', 'firmasite' ); ?></p>
						</div><!-- .main-column -->
					...
					<?php if ( 'crop-image' == bp_get_avatar_admin_step() ) : ?>
						...
						<h3 class="page-header"><?php _e( 'Crop Group Avatar', 'firmasite' ); ?></h3>
				...
				<?php if ( bp_is_group_creation_step( 'group-invites' ) ) : ?>
							...
							<?php if ( bp_group_has_invites() ) : ?>
								...
								<?php while ( bp_group_invites() ) : bp_group_the_invite(); ?>
									...
									<li id="<?php bp_group_invite_item_id(); ?>">
										...
										<span class="activity label label-info"><?php bp_group_invite_user_last_active(); ?></span>
					...
					<?php else : ?>
						...
						<div id="message" class="info">
							<p><?php _e( 'Once you have built up friend connections you will be able to invite others to your group. You can send invites any time in the future by selecting the "Send Invites" option when viewing your new group.', 'firmasite' ); ?></p>
						</div>
				...
				<?php if ( 'crop-image' != bp_get_avatar_admin_step() ) : ?>
					...
					<div class="submit" id="previous-next">
						...
						<?php if ( !bp_is_first_group_creation_step() ) : ?>
							...
							<input type="button" class="btn"  value="<?php _e( 'Back to Previous Step', 'firmasite' ); ?>" id="group-creation-previous" name="previous" onclick="location.href='<?php bp_group_creation_previous_link(); ?>'" />
						...
						<?php if ( !bp_is_last_group_creation_step() && !bp_is_first_group_creation_step() ) : ?>
							...
							<input type="submit" class="btn  btn-primary" value="<?php _e( 'Next Step', 'firmasite' ); ?>" id="group-creation-next" name="save" />
						...
						<?php if ( bp_is_first_group_creation_step() ) : ?>
							...
							<input type="submit" class="btn  btn-primary" value="<?php _e( 'Create Group and Continue', 'firmasite' ); ?>" id="group-creation-create" name="save" />
						...
						<?php if ( bp_is_last_group_creation_step() ) : ?>
							...
							<input type="submit" class="btn  btn-primary" value="<?php _e( 'Finish', 'firmasite' ); ?>" id="group-creation-finish" name="save" />
